...
@pytest.mark.parametrize('api', [create_app(pretty=True)])
...
@pytest.mark.parametrize('api', [create_app(pretty=False)])
...
@pytest.mark.parametrize('api', [create_app(get_context=lambda:"CUSTOM CONTEXT")])
def test_supports_pretty_printing_001(client):
...
@pytest.mark.parametrize('api', [create_app(batch=True)])
